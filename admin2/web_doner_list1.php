                                ...
                                <?php
                                     if(isset($_GET['go'])=="Go"){
                                      if($_SERVER['REQUEST_METHOD'] == "GET"){
                                          include('conn.php');
                                          extract($_GET);
                                          $q="SELECT * FROM all_doner_request_list WHERE doner_name='$data' OR blood_group='$data' OR disease='$data'";
                                          ...
                                          while($row=mysqli_fetch_assoc($res)){
                                              ...
                                              echo "<td>".$row['manager_staff']."</td></tr>";
                                              $i++;
                                          }
                                          $num=mysqli_num_rows($res);
                                          if($_GET['data']=="" || $num==0){
                                            echo "No Records Found <a href='web_doner_list.php'><button class='btn btn-primary btn-sm'>Back</button></a>";
                                          }
                                      }
                                  }
                                  else{
                                  include('conn.php');
                                  $q="SELECT * FROM `all_doner_request_list`";
                                  $res=mysqli_query($conn,$q);
                                  $i=1;
                                  while($row=mysqli_fetch_assoc($res)){
                                    echo "<tr><td>".$i."</td>";
